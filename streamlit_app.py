...
@st.cache_data
def load_data():
    # Try multiple encodings and quoting styles to handle different CSV formats
    for enc in ['utf-8', 'utf-8-sig', 'latin-1']:
        try:
            ...
            if 'Time Frame' in df.columns:
                df['Time Frame'] = pd.to_datetime(df['Time Frame'].astype(str).str.strip().str.strip('"'), errors='coerce')
            # Strip quotes from Location column if present
            ...
            continue
    st.error("Could not load data file. Please check detailed-monthly-tla-tenancy.csv exists in the repository.")
    return pd.DataFrame()
...
# Filter CITIES to only those present in data
...
with tab1:
    ...
    if compare_mode and compare_locs:
        ...
        for event, date, color in [('GFC', '2008-09-01', 'gray'), ('Chch EQ', '2011-02-01', 'orange'),
                                    ('COVID-19', '2020-03-01', 'purple'), ('Rate hikes', '2022-03-01', 'red')]:
            fig.add_vline(x=date, line_dash='dash', line_color=color, opacity=0.6,
